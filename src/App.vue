  ...
  <div id="app" class="contain">
    <gameStateStart v-if="uiState === 'start'">
      ...
      <p v-for="option in characterChoices" :key="option">
        <input type="radio" :id="option" :value="option" v-model="characterinput" class="character-choices">
...
export default {
  ...
  computed: {
    ...mapState(["uiState", "questions", "characterChoices", "character", "questionIndex"]),
  },
  methods: {
    pickCharacter() {
      this.$store.commit("PICK_CHARACTER", this.characterinput)
      this.$store.commit("UPDATE_UISTATE", "characterChosen")
    },
